...
The `param_card.dat` generated by madgraph will have a fixed width set for the mediator. This is not the actual width of the mediator, but an arbitrary value set in Feynrules. The `param_card.dat` should be manually modified to 
...
In the in 2411.15073 described scenario fourth and higher generation dark quarks are only produced in the shower as they don't couple to SM quarks. In pythia their masses are set in the same way as the first three generations. The PDG IDs for higher generation dark quarks are `490010i` with `i = 4, 5, ...8`.  
